...
By default, configuration will produce a **stub executable** when SDKs are missing (`SKYMEET_ALLOW_STUB_BUILD=ON`).
Disable stub mode to enforce a hard failure:

```bash
cmake -S . -B build -DSKYMEET_ALLOW_STUB_BUILD=OFF
```
